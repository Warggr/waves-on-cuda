...
struct Shader {
    ...
    Shader(const std::filesystem::path& filename, int shader_type,
           bool spirv = false) {
        ...
        assert(shader_type == GL_FRAGMENT_SHADER or
               shader_type == GL_VERTEX_SHADER);
        shader = glCreateShader(shader_type);
        if(shader == 0) {
            std::stringstream error_msg;
            GLenum error = glGetError();
            error_msg << "Could not create shader: " << get_error_msg(error)
                      << " (" << error << ")";
            throw std::runtime_error(error_msg.str());
        }
        if(spirv) {
        ...
        if(compiled == GL_FALSE) {
            ...
            std::stringstream error_msg;
            error_msg << "Could not compile shader: "
...
unsigned int load_shader_program(const std::filesystem::path& vertex_shader,
                                 std::filesystem::path fragment_shader) {
    const unsigned int shader_program = glCreateProgram();
    glAttachShader(shader_program,
                   Shader(vertex_shader, GL_VERTEX_SHADER, true));
    glAttachShader(shader_program,
                   Shader(fragment_shader, GL_FRAGMENT_SHADER, true));
...
void MyGLFW::initialize() {
    ...
    glfwMakeContextCurrent(
        window); // This needs to be before we initialize GLEW
    ...
    const unsigned int shader_program =
        load_shader_program("res/vertex_shader.spv", "res/fragment_shader.spv");
    ...
    glBufferData(GL_UNIFORM_BUFFER, sizeof(glm::mat4) * 3, nullptr,
                 GL_DYNAMIC_DRAW); // Allocate memory
...
void Renderer2D::set_grid(const GridView<double, 2>* grid_to_render) {
    GLfloat triangles[grid_to_render->shape()[0] - 1]
                     [grid_to_render->shape()[1] - 1][2][3][3];
    ...
    for(int i = 1; i < grid_to_render->shape()[0]; i++) {
        for(int j = 1; j < grid_to_render->shape()[1]; j++) {
            const float topLeft[3] = {(i - 1) * grid_dx, (j - 1) * grid_dy,
                                      static_cast<float>(
                                          (*grid_to_render)[i - 1][j - 1])},
                        topRight[3] = {(i - 1) * grid_dx, j * grid_dy,
                                       static_cast<float>(
                                           (*grid_to_render)[i - 1][j])},
                        bottomLeft[3] = {i * grid_dx, (j - 1) * grid_dy,
                                         static_cast<float>(
                                             (*grid_to_render)[i][j - 1])},
                        bottomRight[3] = {
                            i * grid_dx, j * grid_dy,
                            static_cast<float>((*grid_to_render)[i][j])};
    ...
    set_triangles(std::span(reinterpret_cast<GLfloat*>(triangles),
                            sizeof(triangles) / sizeof(GLfloat)));
}
...
void MyGLFW::render() {
    ...
    glm::mat4 proj[3] = {
        ...
        glm::translate(glm::scale(glm::mat4(1.0f), glm::vec3(1.0f, 1.0f, 1.0f)),
    ...
    // Upload data to UBO
    glBindBuffer(GL_UNIFORM_BUFFER, ubo);
    glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(proj),
                    glm::value_ptr(proj[0]));
