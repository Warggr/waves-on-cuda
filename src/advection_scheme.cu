...
__global__ void cuda_step(const double* in, PlainCGrid out, double t, double c, std::size_t grid_width, std::size_t grid_height, std::size_t block_size) {
...
void UpwindScheme::step(
    const ::Grid<double, 2>& before,
    ::Grid<double, 2>& after,
    double t,
    double dt
) const {
...
void UpwindScheme::multi_step(
    unsigned int N,
    UpwindScheme::Grid& before,
    UpwindScheme::Grid& after,
    double t,
    double dt
) const {
    const double c = WAVE_SPEED * before.shape()[0] * dt;
