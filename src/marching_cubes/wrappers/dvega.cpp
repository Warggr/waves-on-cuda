...
int* getTriangle(surface* S, int n) {
    return S->T[n >> _MC_N][n & _MC_A];
}

float* getVertex(surface* S, int n) {
    return S->V[n >> _MC_N][n & _MC_A];
}

float* getNormal(surface* S, int n) {
    return S->N[n >> _MC_N][n & _MC_A];
}
...
std::vector<Triangle<float>> marching_cubes(const GridView<double, 3>& grid,
    ...
    // This is also a workaround for the lib, which sets this to -1 if there are
    // no triangles
    if(S->nT == -1)
        return result;
