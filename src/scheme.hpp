...
public:
    using Grid = GridType;
    virtual void step(const GridType& before, GridType& after, double t, double dt) const = 0;
    virtual void multi_step(unsigned int N, GridType& before, GridType& after, double t, double dt) const {
...
public:
    ...
    inline void reset(const Grid& original_grid) { *current_grid = original_grid; }
};
