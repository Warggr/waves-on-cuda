    ...
    for fixed_dim in range(3):
        for fixed_value in (0, 1):
            adj_thisface = [
                ...
                for corner in get_four_corners(
                    fixed_dim=fixed_dim, fixed_value=fixed_value
                )
            ]
...
def get_four_corners(
    fixed_dim: int, fixed_value: int
) -> Generator[Point3D, None, None]:
...
def chain_cube(perm1, perm2) -> CubeRotation:
    return CubeRotation(
        face_permutation=chain_permutation(
            perm1.face_permutation, perm2.face_permutation
        ),
        edge_permutation=chain_permutation(
            perm1.edge_permutation, perm2.edge_permutation
        ),
        vertex_permutation=chain_permutation(
            perm1.vertex_permutation, perm2.vertex_permutation
        ),
    )
