...
def draw_cube(ax, corners: list[float] | None = None):
    ...
    if corners is not None:
        ...
        kwargs["vmax"] = 1
    ax.scatter(
        index_to_corner[:, 0], index_to_corner[:, 1], index_to_corner[:, 2], **kwargs
    )
...
def get_marching_cube_triangles(
    v: list[float], isoLevel: float = 0
) -> tuple[np.ndarray, list[tuple[int, int, int]], str, str]:
    ...
    intersect = permute(
        intersect,
        cube_geometry().all_permutations[case_ptr.permutation].edge_permutation,
    )
    v = permute(
        v, cube_geometry().all_permutations[case_ptr.permutation].vertex_permutation
    )
    ...
    intersect = permute(
        intersect,
        cube_geometry().all_permutations[subcase_ptr.permutation].edge_permutation,
    )
    ...
    for i in range(subcase.num_triangles):
        ...
        for index in triangle_of_indices:
            assert np.all(0 <= intersect[index]) and np.all(intersect[index] <= 1), (
                index,
                intersect,
            )
    return (
        intersect,
        triangles,
        f"Case {case_ptr._case}",
        f"subcase {subcase_ptr.subcase}",
    )


def plot_marching_cube(ax: Axes3D, v: list[float], isoLevel: float = 0) -> None:
    intersect, triangles, case, subcase = get_marching_cube_triangles(
        v, isoLevel=isoLevel
    )
...
def slider_panel(
    ...
    startx=0.1,
    width=0.03,  # in figure coordinates
) -> list[Slider]:
    if initial is None:
        initial = np.zeros(n)
    sliders = []
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                intersect, triangles, _, _ = get_marching_cube_triangles(
                    v, isoLevel=isoLevel
                )
                if len(triangles) == 0:
                    continue
...
def interactive_plot(
    plot_functions: Sequence[Callable[[Axes3D, list[float], float], None]] = [
        plot_marching_cube,
        plot_trilinear,
    ],
):
    ...
    sliders = slider_panel(
        fig, NB_VERTICES, vmin=-1, vmax=1, initial=initial_v, on_change=update
    )
